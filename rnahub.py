...
def print_query_file_contents(query_path):
    try:
        with open(query_path, 'r') as query_file:
            print('query file contents:')
            print(query_file.read().rstrip())
    except OSError as exc:
        logging.warning(f'Unable to read query file {query_path}: {exc}')

def search(seq_path, seq_flanked_path = ''):
    """Run the nhmmer search pipeline and report whether downstream work should continue."""
    global nofiterations
    query = seq_path
    print(f'query: {seq_path}')
    print_query_file_contents(seq_path)
...
def rscape_infernal():
    ...
    try:
       ...
       pass
    exe(f"{RSCAPE_PATH} -s --outdir {job_path}/rscape_infernal --cacofold --outtree {job_path}/infernal.sto | tee {job_path}/rscape_infernal_results.txt", dry)

def cleanup_r2r_outputs(folder_path):
    """Remove R2R artifacts generated by R-scape runs."""
    removable_suffixes = (".R2R.sto", ".R2R.sto.pdf")
    for output_dir in ("rscape_output", "rscape_infernal"):
        output_path = os.path.join(folder_path, output_dir)
        if not os.path.isdir(output_path):
            continue
        for name in os.listdir(output_path):
            if not name.endswith(removable_suffixes):
                continue
...
if __name__ == '__main__':
    ...
    for f in args.input:
        ...
        cmd = ' '.join([f'{EASEL_PATH}/esl-alistat {job_path}/v{nofiterations}_rm.sto > {job_path}/v_last_stats.txt'])
        exe(cmd, dry) 
